	...
	for i, j := 0, 0; i < len1 || j < len2; i, j = i+width1, j+width2 {
		...
		if r1 != r2 {
			if removalSeen {
				...
				// If rune was not equal, then keep the smaller string at the
				// same place and compare with next letter - it should be equal
				// if the two strings are one removal away.
				width1 = 0
				removalSeen = true
			}
		}
...
func AreOneEditAway(input1, input2 string) bool {
	...
	if len1 == len2 { // Check if edit is a replacement.
		var width1, width2 int
		var r1, r2 rune
		diffSeen := false
		for i, j := 0, 0; i < len1 || j < len2; i, j = i+width1, j+width2 {
		...
		return true
	} else if len1-1 == len2 { // Input1 is a removal away from input2.
		return OneRemovalAway(input2, input1)
	} else { // Input2 is a removal away from input1.
